...
function applyLinks() {
  ...
  // Remove Google Ads placeholders like {LOCATION(City)} or LOCATIONCityBrandenburg
  const city = rawCity.match(/^[a-zA-ZäöüÄÖÜß\s\-]+$/) && rawCity.length < 50
    ? rawCity.trim()
    : '';
  ...
  document.querySelectorAll('.wa-l, .whatsapp-link').forEach(el => {
    ...
    el.addEventListener('click', function() {
      ...
      if (typeof gtag !== 'undefined') gtag('event', 'click', { event_category: 'WhatsApp', event_label: path });
    });
  });
  ...
  document.querySelectorAll('.call-l').forEach(el => {
    ...
    el.addEventListener('click', function() {
      ...
      if (typeof gtag !== 'undefined') gtag('event', 'click', { event_category: 'Phone', event_label: path });
    });
  });
  ...
  document.querySelectorAll('.faq-item').forEach(el => {
    ...
    if (panel) { panel.style.display = 'block'; panel.style.gridTemplateRows = '1fr'; }
    if (chev) chev.style.display = 'none';
  });
  document.querySelectorAll('.faq-inner').forEach(el => { el.style.overflow = 'visible'; });
}
applyLinks();

// Form conversion - fires ONLY after Formspree success
